...
# 2b
awk -F "[\t]" 'BEGIN{print "Daftar nama customer di Albuquerque pada tahun 2017 antara lain: "}/Albuquerque/ {if(index($2, "2017") != 0) a[$7]++} END {for (b in a) { print b }; printf"\n";}' $DIR >> $NAMA_FILE
...
# 2d
awk -F "[\t]" 'BEGIN {Reg["Central"]=0; Reg["West"]=0; Reg["South"]=0; Reg["East"]=0; min=1000000;} NR>1 {for(i in Reg) if($13==i) Reg[i] = $(NF); } END {for(i in Reg) if(min>Reg[i]) {min=Reg[i]; min_reg=i;} print "Wilayah bagian (region) yang memiliki total keuntungan (profit) yang paling sedikit adalah "min_reg " dengan total keuntungan " min "\n"; }' $DIR >> $NAMA_FILE